...
public class FluidNetworkDrawer extends ModWorldDrawer {
	...
	public void drawFlowPreview() {
		...
		FluidSystemModule fluidModule = (FluidSystemModule) container.getModMCModule(ElementRegistry.FLUID_TANK.getId());
		if(fluidModule == null) return;
		...
		if(isFluidNetworkBlock(currentType)) {
			...
			FluidSystemModule.FluidNetwork network = getCurrentLookedAtNetwork();
			drawHudInfo(usableController, fluidModule, currentSection, network, currentIndex, currentType);
		}
	...
	private void drawHudInfo(ManagedUsableSegmentController<?> usableController, FluidSystemModule fluidModule, Set<Long> currentSection, FluidSystemModule.FluidNetwork currentNetwork, long currentIndex, short currentType) {
		...
		if(currentType == ElementRegistry.PIPE_PUMP.getId()) {
			// If we're looking at a pump, also show its individual flow contribution
			float pumpFlow = fluidModule.getFlow(currentIndex);
			if(pumpFlow == 0) {
				text += "\nPump Flow: No flow";
			} else if(pumpFlow > 0) {
	...
	public boolean shouldDrawFlowPreview() {
		...
		return GameClient.getPICM().isInAnyStructureBuildMode() && (isFluidNetworkBlock(selectedType) || PlayerInteractionControlManager.isAdvancedBuildMode(GameClient.getClientState())) && BuildModeDrawer.currentPiece != null;
	}
