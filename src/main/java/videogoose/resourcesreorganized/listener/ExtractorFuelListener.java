...
/**
 * Heliogen fuel integration for RRS extractors. Registered as a plain
 * {@link FactoryManufactureListener} so the game's dispatch loop calls both
 * methods directly — no mixin required.
 * <p>
 ...
 * Writes the combined total to {@link FuelTickState#availableFuelUnits}.
 * The inventory is guaranteed live here; the tank module may be null if the
 * system isn't installed/loaded, in which case only canisters are counted.
 * <p>
 ...
 * Skips units already spent by {@link HarvesterEnhancerOverrideListener}
 * (tracked in {@link FuelTickState#spentFuelUnits}).
 * The bonus output is applied here proportional to total fuel consumed this cycle.
 * <p>
 * NOTE: {@code onProduceItem} fires for both input ingredients consumed and output
 * items produced (two separate loops in the vanilla produce() method). Extractors
 * have no recipe inputs, so the input loop never triggers here — the
 * {@code isExtractor} guard is sufficient.
 */
public class ExtractorFuelListener implements FactoryManufactureListener {
	...
	public boolean onPreManufacture(FactoryCollectionManager factoryCollectionManager, Inventory inventory, LongOpenHashSet[] longOpenHashSets) {
		...
		if(block == null || !isExtractor(block.getType())) return true;
		...
		if(ConfigManager.isDebugMode()) {
			// Add fake canisters to the entity's virtual cache so draining uses the normal code-path
			// and writeBackToLive will convert them to empties in the real inventory.
			int fakeCanisters = 10; // 10 canisters worth by default
			EntityFuelManager.getOrCreate(uid).canisterCount += fakeCanisters;
			// Update the FuelTickState to reflect the mutated cache
	...
	public void onProduceItem(RecipeInterface recipeInterface, Inventory inventory, FactoryProducerInterface producer, FactoryResource product, int quantity, IntCollection changeSet) {
		...
		// Always perform a real drain so debug mode mutates the virtual cache and produces empties.
