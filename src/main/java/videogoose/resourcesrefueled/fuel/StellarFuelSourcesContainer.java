...
public class StellarFuelSourcesContainer implements Serializable {
	...
	public StellarFuelSupplier getOrCreate(Vector3i systemPos) {
		...
		ResourcesRefueled.getInstance().logInfo("[ResourcesRefueled] Created StellarFuelSupplier for system " + systemPos + " (" + cls + ", regen=" + regenRate + "/s)");
		return supplier;
	}
	...
	private static SystemClass getSystemClass(Vector3i systemPos) {
		try {
			...
			ResourcesRefueled.getInstance().logInfo("[ResourcesRefueled] Could not resolve SystemClass for " + systemPos + ", using NORMAL.");
		}
